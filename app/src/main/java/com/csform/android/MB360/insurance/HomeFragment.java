...
import static com.csform.android.MB360.BuildConfig.DOWNLOAD_BASE_URL;
...
import com.csform.android.MB360.databinding.FragmentHomeBinding;
import com.csform.android.MB360.fitness.FitnessDashBoardActivity;
...
import com.csform.android.MB360.insurance.enrollment.repository.SummaryViewModel;
...
import com.csform.android.MB360.utilities.LoadingInsuranceDialogue;
...
public class HomeFragment extends Fragment implements DashboardItemClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        providerNetworkModel.setDashBoardImage(ContextCompat.getDrawable(requireContext(), R.drawable.networkhospital));
        providerNetworkModel.setDashBoardHeader(getString(R.string.provider_network));

        if (dashBoardItemList.isEmpty()) {
            dashBoardItemList.add(new DashBoardModel(getString(R.string.my_coverages), ContextCompat.getDrawable(requireContext(), R.drawable.mycoverage), ""));
            ...
            dashBoardItemList.add(new DashBoardModel(getString(R.string.my_claims), ContextCompat.getDrawable(requireContext(), R.drawable.myclaims), ""));
            dashBoardItemList.add(new DashBoardModel(getString(R.string.my_queries), ContextCompat.getDrawable(requireContext(), R.drawable.myquery), ""));
            dashBoardItemList.add(new DashBoardModel("Claim \nProcedures", ContextCompat.getDrawable(requireContext(), R.drawable.claimprocedure), ""));
            dashBoardItemList.add(new DashBoardModel("Intimate \nClaim", ContextCompat.getDrawable(requireContext(), R.drawable.intimateclaim), ""));
            dashBoardItemList.add(new DashBoardModel("Policy \nFeatures", ContextCompat.getDrawable(requireContext(), R.drawable.policyfeature), ""));
            dashBoardItemList.add(new DashBoardModel("FAQs", ContextCompat.getDrawable(requireContext(), R.drawable.faq), ""));

            dashBoardItemList.add(new DashBoardModel("Claim Data Upload Web", ContextCompat.getDrawable(requireContext(), R.drawable.myclaims), ""));
            dashBoardItemList.add(new DashBoardModel("Claim Data Upload App", ContextCompat.getDrawable(requireContext(), R.drawable.myclaims), ""));
        }
...
*/
                        ...
                        binding.cardEnroll.setOnClickListener(v -> {
                            ...
                            if (CARD_ENROLL_ENABLE)
                               ...
                               /*     LoadSessionResponse loadSessionResponse = loadSessionViewModel.getLoadSessionData().getValue();
                                    ...
                                    binding.cardEnroll.setOnClickListener(new View.OnClickListener() {
                                        @Override
    ...
    /*    enrollmentStatusViewModel.getEnrollmentStatusData().observe(getViewLifecycleOwner(), enrollmentStatusResponse -> {
            LogMyBenefits.d("ENROLLMENT-STATUS", "CHANGED");
...
*/
       ...
       /* enrollmentViewModel.getWindowPeriod().observe(getViewLifecycleOwner(), windowPeriod -> {
            if (windowPeriod != null) {
                ...
                try {
                    CountDownTimer timer = windowPeriodCounter.showDays(binding.daycount);
                    if (timer != null) {
                        //we get the window period days here.
                        binding.daycount.setVisibility(View.VISIBLE);
                        binding.daysLeft.setVisibility(View.VISIBLE);
                        binding.dependantImage.setVisibility(View.GONE);
                        timer.start();
                        binding.enrollmentStatus.setText("OPEN");
                    } else {
                        binding.daycount.setVisibility(View.GONE);
                        binding.daysLeft.setVisibility(View.GONE);
                        binding.dependantImage.setVisibility(View.VISIBLE);
                        binding.enrollmentStatus.setText("CLOSED");
                    }
...
*/
      ...
      /*  binding.cardEnroll.setOnClickListener(v -> {
            //faking the handle app session
           *//* try {
                LoadSessionResponse loadSessionResponse = loadSessionViewModel.getLoadSessionData().getValue();

                if (loadSessionResponse != null) {
                    String EmpSrno = loadSessionResponse.getGroupPoliciesEmployees().get(0).getGroupGMCPolicyEmployeeData().get(0).getEmployeeSrNo();
                    String enrollment_url = "https://portal.mybenefits360.com/" + "HandleAppSession.ASPX?APPLOGIN=" + "95685";

                    NavDirections actions = HomeFragmentDirections.actionHomeFragmentToEnrollmentWebView(enrollment_url);
                    navController.navigate(actions);
                ...
                LogMyBenefits.e("EMPLOYEE_ENROLLMENT_ERROR", e.getLocalizedMessage());
                e.printStackTrace();
            }*//*
    ...
    private void download_summaryFile(String url) {
        new DownloadFile(getContext(), getActivity())
                .downloadFilePDF(String.valueOf(System.currentTimeMillis()) + ".pdf", url.toString());
    }
    ...
    public void onDashboardItemClicked(String menuName) {
        ...
        switch (menuName.toLowerCase()) {
            ...
            case "claim data upload app":
                ...
                actions = HomeFragmentDirections.actionHomeFragmentToAllCduClaims();
                navController.navigate(actions);
                break;
